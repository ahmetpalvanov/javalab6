...
public class BouncingBall implements Runnable {
	...
	public BouncingBall(Field field) {
		...
		this.field = field; // Радиус мяча случайного размера
		radius = new Double(Math.random() * (MAX_RADIUS - MIN_RADIUS)).intValue() + MIN_RADIUS; // Абсолютное значение
																								// скорости зависит от
																								// диаметра мяча, // чем
																								// он больше, тем
																								// медленнее
		...
		Thread thisThread = new Thread(this); // Запускаем поток
		thisThread.start();
	}
	...
	public void run() {
		try {
			...
			while (true) {
				...
				field.canMove(this);
				field.stopMiniBalls(this, radius);
				if (x + speedX <= radius) {
			...
			// Скорость = 1 (медленно), засыпаем на 15 мс. // Скорость = 15 (быстро),
			// засыпаем на 1 мс.
